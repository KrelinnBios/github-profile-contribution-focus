...
GitHub Profile Contribution Focus 是一個可重複使用的 GitHub Action。它讀取 GitHub 個人主頁「Last year」範圍內的可見貢獻，依儲存庫和月份產生 SVG 時間帶，呈現這一年主要參與了哪些專案，以及貢獻重心如何變化。

圖表只顯示貢獻量最高的 5 個儲存庫，其餘儲存庫和無法安全歸屬到特定儲存庫的貢獻逐月彙總為 `Other`。產生結果保存在自己的個人主頁儲存庫中，不依賴外部圖片服務。

## 功能概覽

- GitHub「Last year」時間帶：直接採用個人主頁的預設統計範圍，通常跨越 13 個自然月。
- 儲存庫貢獻列：每列對應一個參與過的可見儲存庫，每個色塊表示該儲存庫當月的全部貢獻量。
- 相對強度：使用 4 個強度等級區分活躍程度，空月份顯示為主題色軌道。
- 貢獻重心：依 GitHub「Last year」範圍內的總貢獻量顯示前 5 個儲存庫，其餘內容逐月彙總為 `Other`。
...
<p align="center">
  <img src="examples/preview.svg" alt="GitHub Last year 範圍內的儲存庫貢獻重心圖">
</p>
...
範例工作流程每 6 小時自動執行，也支援手動執行；每次執行都會解析最新正式版本，無需設定跨儲存庫 Token。
...
- 排名依 GitHub「Last year」範圍內的總貢獻量計算，`Other` 則逐月彙總未進入前 5 名或無法歸屬的貢獻。
